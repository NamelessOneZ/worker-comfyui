...
def handler(job):
    ...
    try:
        ...
        for node_id, node_output in outputs.items():
            # Handle video outputs (from SaveVideo node)
    ...
    # Separate images and videos in the output
    images = [item for item in output_data if not item.get("filename", "").endswith((".mp4", ".webm", ".mov", ".avi"))]
    videos = [item for item in output_data if item.get("filename", "").endswith((".mp4", ".webm", ".mov", ".avi"))]
    ...
    print(f"worker-comfyui - Job completed. Returning {len(images)} image(s) and {len(videos)} video(s).")
    return final_result
